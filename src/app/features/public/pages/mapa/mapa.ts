...
export class MapaComponent implements AfterViewInit, OnDestroy {
  ...
  tiposFiltro = [
    { key: 'eventos', label: 'Eventos', icon: '📅', checked: true, color: '#0a7e6f' },
    { key: 'servicios', label: 'Servicios', icon: '🔧', checked: true, color: '#3b82f6' },
    { key: 'lugares', label: 'Lugares Turísticos', icon: '🏛️', checked: true, color: '#f59e0b' },
    { key: 'denuncias', label: 'Denuncias', icon: '🚧', checked: true, color: '#dc2626' },
    { key: 'paradas', label: 'Paradas de Bus', icon: '🚏', checked: true, color: '#8b5cf6' }
  ];
      ...
      next: (resp) => {
        ...
        eventos.forEach((e: any) => {
          if (e.location?.coordinates?.length === 2) {
            ...
            this.agregarMarcador(lat, lng, e.title, e.description, '📅', '#0a7e6f');
          }
        });
      },
      error: () => {}
      ...
      next: (resp) => {
        ...
        servicios.forEach((s: any) => {
          if (s.location?.coordinates?.length === 2) {
            ...
            this.agregarMarcador(lat, lng, s.name, s.description, '🔧', '#3b82f6');
          }
        });
      },
      error: () => {}
      ...
      next: (resp) => {
        ...
        lugares.forEach((l: any) => {
          if (l.location?.coordinates?.length === 2) {
            ...
            this.agregarMarcador(lat, lng, l.name, l.description, '🏛️', '#f59e0b');
          }
        });
      },
      error: () => {}
      ...
      next: (resp) => {
        ...
        denuncias.forEach((d: any) => {
          if (d.location?.coordinates?.length === 2) {
            ...
            this.agregarMarcador(lat, lng, d.title, d.description, '🚧', '#dc2626');
          }
        });
      },
      error: () => {}
      ...
      next: (resp) => {
        ...
        paradas.forEach((p: any) => {
          if (p.location?.coordinates?.length === 2) {
            ...
            this.agregarMarcador(lat, lng, p.name, p.address || 'Parada de bus', '🚏', '#8b5cf6');
          }
        });
      },
      error: () => {}
  ...
  private agregarMarcador(lat: number, lng: number, titulo: string, desc: string, icono: string, color: string) {
    ...
    try {
      const icon = L.divIcon({
        html: `<div style="background:${color};border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-size:14px;color:white;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);">${icono}</div>`,
        className: '',
        iconSize: [28, 28]
      });
